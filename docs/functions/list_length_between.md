...
page_title: "list_length_between function - terraform-provider-validatefx"
subcategory: ""
description: |-
...
Returns true when the list length is within the specified range (inclusive).
...
```text
list_length_between(values list of string, min string, max string) bool
```
...
1. `min` (String) Minimum length (inclusive).
1. `max` (String) Maximum length (inclusive).
