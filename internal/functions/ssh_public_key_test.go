...
func TestSSHPublicKeyFunction(t *testing.T) {
	t.Parallel()

	fn := NewSSHPublicKeyFunction()
	...
	cases := []struct {
		...
		{name: "valid", value: types.StringValue("ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIKJf0N0nH7kz5Zr4xkz0GWWJrPq9uO2m6sR3j0s8v2QG test@example"), expectTrue: true},
		{name: "invalid", value: types.StringValue("not-a-key"), expectError: true},
